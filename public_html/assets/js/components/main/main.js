...
import AnchorTitle from '../common/anchorTitle';
...
class Main {
  constructor(data) {
    ...
    nunjucks.render('main.html', json, (err, res) => {
      ...
      this.$dashboardModeMobile = $('<div class="dashboard-mode-mobile"></div>');
      ...
      new AnchorTitle($('#settings__value--BoxName'));
      new AnchorTitle($('.dashboard-keys'));
  ...
  movingMode(changes) {
    if (changes) {
      if (this.device.name === 'desktop') {
        this.$dashboardMode.closest('.dashboard-mode').insertAfter('.dashboard-location');
